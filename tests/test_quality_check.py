...
from ask_the_web.src.quality_check import extract_citations, validate_citations
...
@patch("ask_the_web.src.quality_check.genai.GenerativeModel")
def test_validate_citations_success(mock_model):
    ...
    mock_instance.generate_content.return_value.text = (
        "Sentence 1, Citation [1]: YES - The source explicitly mentions stress reduction."
    )
