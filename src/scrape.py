...
@st.cache_data
def scrape_page(url: str, max_retries: int = 3, backoff_factor: float = 1.5) -> Optional[str]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not main_content:
                ...
                main_content = " ".join(p.get_text(strip=True) for p in paragraphs if len(p.get_text(strip=True)) > 40)
            ...
            # Clean up the text
            main_content = re.sub(r'\s+', ' ', main_content)  # Replace multiple spaces with single space
            main_content = re.sub(r'[\n\r\t]+', ' ', main_content)  # Remove newlines and tabs
            ...
            if hasattr(e, 'response') and e.response.status_code in [403, 404, 429, 500, 502, 503]:
        ...
        print(f"Retrying {url} in {wait_time:.1f} seconds... (Attempt {attempt + 1}/{max_retries})")
